...
class HRRRMiniDataset(DownscalingDataset):
    ...
    def __getitem__(self, idx):
        ...
        raw = self.input[idx].copy()        # (26,8,8)
        x = self.upsample(raw)    
        ...
        # print(f"raw.shape: {raw.shape}, self.input.shape: {self.input.shape}, self.output.shape: {self.output.shape}, normalized output.shape: {y.shape}")

...
def load_model_checkpoint(checkpoint_path, model, device):
    ...
    # DDP로 저장된 경우 혹시 module. prefix가 남아있으면 제거
    new_state_dict = {}
...
def main(cfg: DictConfig) -> None:
    ...
    if cfg.dataset.type == "blastnet":
        dataset, sampler = get_dataset_and_sampler_timeindices(
            dataset_cfg=dataset_cfg, load_all_times=cfg.generation.load_all_times, time_indices=times
        )
        save_time=False
    ...
    logger0.warning(f"Dataset in-image shape: {dataset.input.shape}, out-image shape {dataset.output.shape}, upsmaple facotr {dataset.upsample_factor}")
    ...
    with torch_cuda_profiler:
        with torch_nvtx_profiler:
            ...
            times = dataset.time()
            for dataset_index, (image_tar, image_lr, image_lr_upsampled, *lead_time_label) in zip(
                sampler,
                iter(data_loader),
            ):
                time_index += 1

...
if [ "$REGRESSION" == "swinir" ]; then
  REG_PATH=$(ls -1 "$CHECKPOINTS_DIR/checkpoints_regression"/swinir_step_*.pt \
    | sort -V \
    | tail -n 1)
fi
...
if [[ ! -f "${SOURCE_DATASET}" || ! -f "${TARGET_DATASET}" ]]; then
  echo "Residual dataset not found. "
...
if $GENERATE_DATA; then
  ...
  if [ "$REGRESSION" == "swinir" ]; then
    python -m rematch.generate_swinir --config-name="${CONFIG}" \
      ++generation.io.reg_ckpt_filename=$REG_PATH \
      ++generation.io.output_filename=$SOURCE_DATASET \
      ++generation.num_ensembles=1 \
      ++generation.inference_mode="regression"\
